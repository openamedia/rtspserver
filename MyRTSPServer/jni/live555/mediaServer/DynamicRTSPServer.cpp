...
#define LOG_TAG "DynamicRTSPServer"
...
DynamicRTSPServer::createNew(UsageEnvironment& env, Port ourPort,
  ...
  return new DynamicRTSPServer(env, ourSocket, ourPort, authDatabase, pool, reclamationTestSeconds);
}

DynamicRTSPServer::DynamicRTSPServer(UsageEnvironment& env, int ourSocket,
									 Port ourPort,
									 UserAuthenticationDatabase* authDatabase, H264LiveFramePool* pool, unsigned reclamationTestSeconds)
...
static ServerMediaSession* createNewSMS(UsageEnvironment& env,
										char const* fileName, FILE* fid, H264LiveFramePool* pool); // forward
...
static ServerMediaSession* createNewSMS(UsageEnvironment& env,
										char const* fileName, FILE* fid, H264LiveFramePool* pool) {
	ALOGE("createNewSMS fileName %s", fileName);

  ServerMediaSession* sms = NULL;
  Boolean const reuseSource = False;

  if(strcmp(fileName, "recorderLive") == 0){
	  NEW_SMS("H.264 Video Recorder Live");
	  OutPacketBuffer::maxSize = 100000;
	  sms->addSubsession(H264LiveVideoServerMediaSubssion::createNew(env, fileName, reuseSource, pool));
	  return sms;
  }
  ...
  if (strcmp(extension, ".aac") == 0) {
    ...
    NEW_SMS("H.264 Video");
	ALOGE("H.264 Video create H264LiveVideoServerMediaSubssion!!");
...
//#define INTERLEAVE_ADUS 1
	...
	ALOGE("createNewSMS MPEG-1 or 2 Audio");
